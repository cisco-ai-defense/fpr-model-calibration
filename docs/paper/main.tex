...
\title{Full-range Binary Classifier Calibration for Safe Model Updates in Production}
...
\begin{abstract}
  ...
  On data never seen during fitting, held-out relative FPR error stays within 2.3\% from 0.1\% to 10\% FPR and within 7.2\% at 0.01\% FPR.
  The shipped artifact is around 200 KB and uses compiled interpolation over 10K knots per score, independent of calibration-set size.
\end{abstract}
...
Detection models running in adversarial environments face a malicious distribution that drifts rapidly while the benign distribution stays comparatively stable, so teams retrain and redeploy constantly to stay ahead of new threats \citep{swanda2025}.
Retraining tends to change the output prediction scores, which breaks downstream users of the model.
For these security-oriented models we need consistent false-positive performance across all output values.
Standard probability-calibration methods such as Platt scaling and isotonic regression calibrate class probability rather than an FPR contract \citep{platt1999,zadrozny2002}.
...
In the update setting we target, benign traffic is the larger and more directly measurable sample pool, while the attack class rate that precision depends on moves with adversary behavior.
Sample-size requirements at a target relative precision follow the benign count, orders of magnitude larger than the predicted-positive count a precision estimator divides by; \cref{sec:reliability} gives the binomial planning rule.

A single detector serves a policy at multiple operating tiers (block at 0.1\% FPR, escalate at 10\%, alert at 1\%), so calibration has to hold at every threshold rather than at a single chosen $\alpha$.
...
\begin{enumerate}
  ...
  \item \textbf{Knot-subsampled, fixed-size deployable artifact.}
    A second isotonic fit on a deterministic log-spaced FPR grid produces a calibration artifact around 200 KB with compiled interpolation over 10K knots per score, independent of calibration-set size.
  ...
  \item \textbf{Release-artifact packaging for cross-retraining stability.}
    The calibration artifact ships alongside the model, so consumers hold thresholds constant and the customer's alert-volume budget is preserved across retraining cycles.
\end{enumerate}

On the Credit Card Fraud Detection benchmark (284{,}807 transactions, 0.172\% positive rate, logistic-regression detector), held-out relative FPR error stays within 2.3\% from 0.1\% to 10\% FPR and within 7.2\% at 0.01\% FPR.
Those values are measured on the subset of eval data never seen during calibration fitting.

% === 2. FPR estimate bounds and biases ===
\section{FPR estimate bounds and biases}
\label{sec:reliability}

How accurately can any method estimate an FPR?
Two finite-sample effects set the ceiling for any calibrator: sampling variance on the benign set, and edge-of-sample bias when rank-selected score thresholds are assigned FPR labels for spline knots.
...
At the common deployment target $p=10^{-3}$, more than 50K benign calibration samples are needed to estimate FPR to within even a rough $\pm25\%$ relative band.
Below that scale, the estimate is dominated by sampling noise from too few benign examples.
...
This correction does not assume uniform model scores.
It applies on the probability scale, where any continuous score distribution becomes uniform after replacing each score by its percentile.
On that scale, $n$ sorted samples create $n+1$ probability gaps, so the $k$-th largest score has mean fresh-sample FPR \citep{nistOrderStatisticMeans}
\begin{equation}
...
The mean correction explains why $k/n$ is high.
...
We use \texttt{IsotonicRegression} because it is sklearn's built-in monotone piecewise-linear spline and fits directly inside an sklearn \texttt{Pipeline}; in this method it is mainly a deployable spline container, not a new statistical model.
...
FPR is multiplicative in practice: $1\%$ to $0.1\%$ and $0.1\%$ to $0.01\%$ are both tenfold reductions in benign alerts.
...
\Cref{tab:anchors} pins memorable values to operating points: $0.5$ means 0.1\% FPR (1 in 1{,}000 benign flagged), $0.7$ means 0.01\% FPR, $0.85$ means 0.001\% FPR, and $1.0$ means ``flag nothing.''
...
\textbf{Scale scores.}
A \texttt{MinMaxScaler} fit on the input domain $[0, 1]$ maps raw scores to $[0, 0.99]$ as a pure multiplicative rescale.
...
The pairs are already monotone, so sklearn stores a linear spline through the observed benign thresholds without changing their order.
...
% === 7. Fixed-size deployable artifact ===
\section{Fixed-size deployable artifact}
\label{sec:artifact}

Production deployment needs a constant-size artifact and a constant-cost inference path, regardless of how much benign data the calibration team used.
An uncapped \texttt{IsotonicRegression} fit stores one knot per unique training score, so 10M benign samples produce an artifact with $\sim 10$M knots in the tens of megabytes and lookup cost proportional to training-set size.
...
A linear spline passes exactly through its knot points, so approximation error is bounded by knot density in the region of interest, and log-spaced sampling concentrates density at low FPR where calibration precision matters.

At 10K knots the two production pipeline steps serialize under \texttt{joblib.dump} to around 200 KB whether the calibration set had 1K or 1M benign samples; we verified the bound across four orders of magnitude in $n$.
...
Deployment pipelines and CDN caches plan around a constant.

% === 8. Evaluation ===
\section{Evaluation on Credit Card Fraud Detection}
\label{sec:evaluation}

We validate on the Credit Card Fraud Detection benchmark, loaded via \texttt{sklearn.datasets.fetch\_openml(name="creditcard", version=1)} (OpenML \texttt{data\_id} 1597; \citealp{dalpozzolo2015}): 284{,}807 European credit-card transactions with 492 fraud cases, a 0.172\% positive rate that sits inside the FP-sensitive regime.
...
\input{figures/credit_card_eval_table.tex}
...
Platt scaling fits a sigmoid to SVM scores so they can be read as posterior class probabilities \citep{platt1999}.
...
Calibration below the calibration-fit set's lowest observed tail FPR value is linear extrapolation from the two lowest observed points.
...
This is a data-freshness concern, not a defect of the calibrator, and it binds regardless of calibration method.
...
The log spacing between anchors should stay.
...
This distinction matters most for safety and security detectors because the attack distribution is adversarial and drifts with every model update.
In the update setting we target, the benign distribution is the larger and more directly measurable sample pool and is directly tied to user harm through false positives.
...
The evaluation shows that a fixed 10K-knot artifact can preserve the calibrated curve when the benign calibration set supports the target FPR range.
On the benchmark, anchor-level held-out relative FPR error stays within 2.3\% from 10\% to 0.1\% FPR and reaches 7.2\% at 0.01\% FPR, while the serialized artifact stays around 200 KB.
...
The sample-size table uses the normal approximation in \cref{eq:normal-binomial-interval} as a planning rule, not as an exact interval claim.
